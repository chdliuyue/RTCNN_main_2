...
# Get layer and gradient w.r.t. loss
    ...
    with tf.GradientTape(persistent=True) as tape:
        pre = model(model_inputs)
        labels2 = tf.convert_to_tensor(labels, dtype=tf.float32)
        loss = model.compiled_loss(labels2, pre)
        beta_gradient = tape.gradient(loss, beta_layer.weights[0])
        for i in range(len(beta_layer.get_weights()[0])):
            a = np.array(np.squeeze(tape.gradient(tf.squeeze(beta_gradient[i]), beta_layer.weights[0]).numpy()))
            Hessian_lines_op.append(a)

# Get second order derivative operators (linewise of Hessian)
    Hessian = np.squeeze(Hessian_lines_op) * data_size
...
#     Hessian=[]
#     func_inputs=[*[inputs for inputs in model_inputs]]
...
def model_summary(trained_model, X_train, Q_train, y_train,
    ...
    if not isinstance(emb_betas_stds, float) and  not isinstance(exog_betas_stds, float):
        ...
        df_stats=pd.DataFrame(index=[i[0] for i in stats_all],
                         data=np.array([[float(i[1]) for i in stats_all],[float(i[2]) for i in stats_all],

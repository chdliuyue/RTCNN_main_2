...
def E_MNL_train(X_train, Q_train, y_train, model,
    ...
    mnl_model = model(NUM_CONT_VARS, NUM_EMB_VARS,
    ...
    if VERBOSE:
        ...
        mnl_model.fit(x=[X_train, Q_train], y=y_train, sample_weight=sample_weight, epochs= N_EPOCHS,
        ...
        mnl_model.fit(x=[X_train, Q_train], y=y_train, epochs= N_EPOCHS, sample_weight=sample_weight,
                   steps_per_epoch= 50, shuffle= 'batch',
                   verbose= VERBOSE, callbacks=[Callback])
...
def EL_MNL_train(X_train, Q_train, y_train, model,
